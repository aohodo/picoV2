...
真实仓库评测与确定性 Harness 回归使用不同口径。真实任务必须保留失败运行，固定任务集与仓库修订，并公开 pass@1、重复运行稳定率、首次写入、工具/模型调用、耗时、token、验证结果及失败分类。统一记录格式和 V1/V2 汇总命令见 [Real-repository coding benchmark](docs/repository-coding-benchmark.md)。
